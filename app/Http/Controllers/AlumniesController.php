...
class AlumniesController extends Controller
{
    ...
    {
        return view('alumnies.dashboard');
    }

    // to get list of alumnies
    public function alumnieList()
    {
        $alumnies = Alumni::where('user_id', '!=', auth()->user()->id)->get();
        ...
        return view('alumnies.alumniSearch.index')->with($data);
    }

    // to get Alumnies Detail Info
    public function alumniInfo($id)
    {
        $user = User::findOrFail($id);
        $alumni = Alumni::findOrFail($user->alumni->id);

        return view('alumnies.forum.alumniDetail')->with('alumni', $alumni);
    }

    // to get alumnies detail
    public function alumniDetail($id)
    {
        $alumni = Alumni::findOrFail($id);

        return view('alumnies.alumniSearch.detail')->with('alumni', $alumni);
    }
    ...
    public function advancedSearch(Request $request)
    {
        ...
        {
            $searchedAlumnies = Alumni::where('gender', $gender)
                                        ->where('department_id', $department)
                                        ->where('user_id', '!=', auth()->user()->id)->get();
        }
        if($gender != NULL & $department == NULL)
        {
            $searchedAlumnies = Alumni::where('gender', $gender)->where('user_id', '!=', auth()->user()->id)->get();
        }
        if($gender == NULL & $department != NULL)
        {
            $searchedAlumnies = Alumni::where('department_id', $department)->where('user_id', '!=', auth()->user()->id)->get();
        }

        // if the searched result returns 0 result
        if(!$searchedAlumnies & $searchedAlumnies != NULL)
        {
            $data = array(
                'departments' => $departments,
        ...
        if($searchedAlumnies != NULL)
        {
            ...
            if($address != NULL)
            {
                ...
                if($workCatagory != NULL)
                {
                    ...
                    $searchedAlumnies = Alumni::whereIn('id', $alumni_ids)->where('user_id', '!=', auth()->user()->id)->get();
